...
{//这里并没有真正解析请求内容，只是检查是否被完整读入，事实上请求报文也很少有内容实体主体
    ...
    {
        //text[m_content_length]='\0';//这里书里面的代码应该是有问题的，这个contentlength都不知道指向哪里
        text[m_content_length+m_checked_idx]='\0';//要加上checked_idx才有意义，即把请求内容的最后位置置空
        return GET_REQUEST;
    }
    return NO_REQUEST;
}
...
bool HttpConn::Write()//通过writev把要发送的东西发送出去
{
    ...
    int bytes_to_send=m_write_idx;//还未发送的字节数
    ...
    while(1)
    {
        ...
        if(bytes_to_send<=bytes_have_send)//如果已发送字节数大于未发送的，说明已经发完了
        {
            Unmap();
